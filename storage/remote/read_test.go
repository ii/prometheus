...
func TestAddExternalLabels(t *testing.T) {
	...
	for i, test := range tests {
		matchers, added := addExternalLabels(test.el, test.inMatchers)
...
func TestPreferLocalFilter(t *testing.T) {
	...
	for i, test := range tests {
		...
		querier, err := fn(ctx, test.mint, test.maxt)
		if err != nil {
			t.Fatal(err)
		}

		if test.querier != querier {
			t.Errorf("%d. expected quierer %+v, got %+v", i, test.querier, querier)
		}
	}
}
